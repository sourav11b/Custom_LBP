 ...
 */
package com.datastax.custom_lbp.customlbp;
 ...
 *
 * <p>
 * <b>Local datacenter</b>: This implementation requires a local datacenter to
 * be defined, otherwise it will throw an {@link IllegalStateException}. A local
 * datacenter can be supplied either:
 *
 * <ol>
 * <li>Programmatically with
 * {@link com.datastax.custom_lbp.session_handling.SessionBuilder#withLocalDatacenter(String)
 * SessionBuilder#withLocalDatacenter(String)};
 * <li>Through configuration, by defining the option
 * {@link DefaultDriverOption#LOAD_BALANCING_LOCAL_DATACENTER
 * basic.load-balancing-policy.local-datacenter};
 * <li>Or implicitly, if and only if no explicit contact points were provided:
 * in this case this implementation will infer the local datacenter from the
 * implicit contact point (localhost).
 * </ol>
 *
 * <p>
 ...
 * shuffled; if more than 2 replicas are available, they will be ordered from
 * most healthy to least healthy ("Power of 2 choices" or busy node avoidance
 * algorithm). Non-replica nodes will be included in a round-robin fashion. If
 * the local datacenter is defined (see above), query plans will only include
 * local nodes, never remote ones; if it is unspecified however, query plans may
 * contain nodes from different datacenters.
 */
@ThreadSafe
public class DefaultLoadBalancingPolicy_Custom extends BasicLoadBalancingPolicy_Custom implements RequestTracker {

	private static final Logger LOG = LoggerFactory.getLogger(DefaultLoadBalancingPolicy_Custom.class);
	...
	public DefaultLoadBalancingPolicy_Custom(@NonNull DriverContext context, @NonNull String profileName) {
		super(context, profileName);
		this.avoidSlowReplicas = profile.getBoolean(DefaultDriverOption.LOAD_BALANCING_POLICY_SLOW_AVOIDANCE, true);
	}
...
//		}
		...
		Object[] replicaNodes = allReplicas.toArray();
			...
			if (replicaCount > 1) {
				...
				if (replicaCount > 2) {
					...
					for (int i = 0; i < replicaCount; i++) {
						...
						if (upTimeNanos != null && now - upTimeNanos - NEWLY_UP_INTERVAL_NANOS < 0
					...
					if (newestUpReplica == null && unhealthyReplicasCount > 0
					...
					// - the replica in first or second position is the most recent replica marked
					// as UP and
					// - dice roll 1d4 != 1
						...
						ArrayUtils.bubbleDown(currentNodes, newestUpReplica == currentNodes[0] ? 0 : 1,
								replicaCount - 1);
					}
...
//
//			Node node = (Node) currentNodes[i];
//			LOG.trace(i+"----"+node.getEndPoint().toString());
//
//		}
...
//
//			Node node = (Node) currentNodes[i];
//			LOG.trace(i+"----"+node.getEndPoint().toString()+"----"+node.getDatacenter());
//
//		}
	...
	protected boolean isResponseRateInsufficient(@NonNull Node node, long now) {
		// response rate is considered insufficient when less than 2 responses were
		// obtained in
	...
	protected int getInFlight(@NonNull Node node, @NonNull Session session) {
		// The cast will always succeed because there's no way to replace the internal
		// session impl
		ChannelPool pool = ((DefaultSession) session).getPools().get(node);
		// Note: getInFlight() includes orphaned ids, which is what we want as we need
		// to account
		// for requests that were cancelled or timed out (since the node is likely to
		// still be
		// processing them).
